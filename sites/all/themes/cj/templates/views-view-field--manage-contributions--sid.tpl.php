...
<?php 
  ...
  // if user can accept
  if (isset($choices[3])) {
    $output[] = l(t('Accept'), 'masry/nojs/contribution-workflow-state/3/' . $row->nid, $options);
  }
  
  // if user can reject
  if (isset($choices[4])) {
    $output[] = l(t('Reject'), 'masry/nojs/contribution-workflow-state/4/' . $row->nid, $options);
  }
  
  // if user can delete
  if (isset($choices[5])) {
    $output[] = l(t('Delete'), 'masry/nojs/contribution-workflow-state/5/' . $row->nid, $options);
  }
